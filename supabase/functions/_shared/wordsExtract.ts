...
3. words — все слова и устойчивые выражения, которые файл даёт для запоминания. Если в файле есть список или таблица слов — берутся слова оттуда, все. Если это связный текст без списка — слова, ради которых этот текст стоит читать; служебные слова (的, 了, 是, 我) не нужны. Каждое слово один раз, в порядке файла.
   ...
   - translation_in_file — перевод, который написан в САМОМ ФАЙЛЕ рядом с этим словом, слово в слово. Если в файле перевода нет — пустая строка. Знак вопроса, прочерк, многоточие, пустая клетка или непонятные символы вместо перевода — это не перевод: тоже пустая строка. Не переводи сам в это поле;
   - meaning_ru — короткий перевод по-русски в том смысле, в каком слово стоит в файле. Всегда, даже если перевод в файле есть.`;
...
/**
 * Перевод — это текст со словами. «?», «—», «…», «___» и прочие символы на
 * месте перевода значат, что ученик его не знал или не дописал, а не
 * перевод: такое поле считается пустым, и перевод предлагает словарь. Модели
 * это сказано в промпте, но держит правило код — модель может переписать
 * «?» из файла как есть.
 */
const TRANSLATION_LETTER = /[\p{Script=Cyrillic}\p{Script=Latin}]/u;

export function translationText(text: string): string | null {
  const clean = clip(text);
  return clean !== null && TRANSLATION_LETTER.test(clean) ? clean : null;
}
