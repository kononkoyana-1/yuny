...
> **2026-09-23: модули и уроки (§7–§10, экран урока в §11) заменены механикой изучения слов** — ежедневные повторения «Сегодня» и учёба по папке своего словаря. Источник правды по ней — `docs/learning/vocabulary-engine.md` и `docs/learning/daily-and-folder-study.md`. Разделы про модули и уроки ниже остаются как история; код серверной части уроков в репозитории не удалён, но не развивается.
...
**Уровень показывается в словаре:** метка «HSK 2» в заголовке статьи слова и в строке выдачи (#80). Решение владельца от 2026-09-25 отменяет прежнее правило п. 4 опроса («в карточках и в словаре уровень не показывается»).
...
> **2026-09-24:** интерфейс логически новый (изучение слов, #57), поэтому текущий вид — не образец. Раскладку и визуальный язык решает `ui-designer`: сочно, современно, по канонам UX/UI. Токены и примитивы ниже — стартовая точка, их можно расширять и заменять (новые токены объявляются в спецификации, вносит `design-system-agent`). Пустую геймификацию (XP, лиги) по-прежнему не делаем.

Стартовая точка: токены в `shared/config/tokens.ts`, примитивы в `apps/mobile/shared/ui` (`Button`, `Card`, `Text`, `Input`, `ProgressBar`, `EmptyState`, `ErrorState`, `LoadingState`, `FeedbackBanner`, `Icon`, `Mascot`, `AtmosphericBackground`, `Sparkles`).

Прежнее направление (clean, calm, spacious) снято решением выше. Не копировать Duolingo. Маскот остаётся элементом оформления — экраном ожидания, пустым состоянием, реакцией на результат, — но перестаёт быть механикой роста.
