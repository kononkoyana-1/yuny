 ...
 * карточек. Что на выходе: примерно 1,9 млн записей — только заголовки длиной
 * до четырёх иероглифов. Длиннее — это фразы, топонимы и имена собственные;
...
async function handle(headword, reading, body) {
  ...
  const readingText = reading && !reading.startsWith("[") ? stripTags(reading) : null;
