...
| Модель агента | `model:` во frontmatter карточки — см. §0.1 |

### **0.1 Какая модель у какой роли**

**Правило (2026-09-09, решение пользователя).** Задачи принимает, анализирует
и принимает по ним решения **Opus**. Реализацию выполняет **Sonnet**.
...
там Opus; где результат — это *артефакт по уже принятому решению*, там Sonnet.
Именно поэтому `ui-designer` на Opus, хотя он «всего лишь» пишет markdown, а
`design-system-agent` на Sonnet, хотя он пишет код.

**Как это работает механически.** Модель задаётся полем `model:` во
frontmatter карточки агента. Параметр `model` в вызове `Agent` tool
**перекрывает** frontmatter — поэтому Orchestrator его не передаёт, кроме
случая, когда пользователь явно просит другую модель для конкретного запуска.
Исключение: `subagent_type: "fork"` всегда наследует модель родителя, и
`model` для него игнорируется.
...
| Цикл «дизайнер → разработчик → ревью» | `docs/DESIGN_LOOP.md` |
...
специализацией. `ui-designer` не дробит реализацию: он выносит наружу то, что
раньше делал никто явно — **дизайн-решение до кода и вердикт по результату**.
До него `frontend-builder` одновременно решал, как экран выглядит, строил его
и сам же признавал готовым, что нарушает принцип 7 (создатель — не
единственный судья качества) ровно так же, как это нарушал бы
`content-agent` без `content-qa-agent`. Границы не пересекаются:
`ui-designer` пишет только в `docs/design/**` и не редактирует ни строки в
`apps/mobile/**`; `design-system-agent` остаётся единственным владельцем
токенов и `shared/ui` — дизайнер может *попросить* примитив, но не сделать
его. Второй фронтенд-агент под роль «разработчик в паре» сознательно **не**
создан: у `frontend-builder` уже есть нужные Boundaries, Validation Pipeline и
память, дублирование запрещено принципом 10 — вместо него в его карточку
добавлена секция **Design Loop**. Протокол пары — `docs/DESIGN_LOOP.md`.
